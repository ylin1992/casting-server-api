...
class TestCastingApi(unittest.TestCase):
    ...
    def test_200_get_actors_by_existing_id(self):
        res = self.client().get('/actors/3', headers=PRODCUER_AUTH_HEADER)
        data = json.loads(res.data)
    ...
    def test_200_get_movies_by_actor_id(self):
        res = self.client().get('/actors/2/movies', headers=PRODCUER_AUTH_HEADER)
        data = json.loads(res.data)
        self.assertEqual(res.status_code, 200)
        self.assertTrue(data['success'])
    ...
    def test_200_pacth_actor_with_normal_input(self):
        res = self.client().patch('/actors/1', headers=PRODCUER_AUTH_HEADER, json={'name':'modified', 'age':10, 'gender': 'f', 'movies': [2,3,4]})
        data = json.loads(res.data)
        ...
        # test appending data
        res2 = self.client().patch('/actors/1', headers=PRODCUER_AUTH_HEADER, json={'movies': [1,2,3,4]})
